...
public class ChatController {
    ...
    @PostMapping
    public ResponseEntity<SuccessResponse<ChatMessageRes>> sendChat(
            @Valid @RequestBody ChatMessageReq chatMessageReq,
            @AuthenticationPrincipal UserPrincipal userPrincipal
    ) {
        ChatMessageRes chatMessageRes = chatService.send(chatMessageReq, userPrincipal);
        return ResponseEntity.status(HttpStatus.OK).body(SuccessResponse.from(chatMessageRes));
    }
